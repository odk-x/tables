...
import org.opendatakit.hope.fragments.TableMapInnerFragment;

import android.app.Activity;
import android.support.v4.app.Fragment;
import android.webkit.WebViewClient;

public class CustomTableView extends CustomView implements
		ExtendedTableControl {
	...
	private void set(UserTable table) {
		...
		for (ColumnProperties cp : elementKeyToColumnProperties.values()) {
			...
			if (smsLabel != null) {
				...
				colIndexTable.put(smsLabel,
						colIndexTable.get(cp.getElementKey()));
			}
		}
	}
	...
	public static CustomTableView get(Activity activity, UserTable table,
		...
		String[][] metadata = new String[indexes.size()][table
				.getNumberOfMetadataColumns()];
		String[] footers = new String[table.getWidth()];
		// Set all the data for the table.
		for (int i = 0; i < table.getWidth(); i++) {
			headers[i] = table.getHeader(i);
	...
	public boolean selectItem(int index) {
		((TableMapInnerFragment) mFragment)
				.focusOnMarker(table.getRowId(index));
		return true;
	}
