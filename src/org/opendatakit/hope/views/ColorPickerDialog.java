...
import org.opendatakit.hope.views.components.WScrollView;
...
public class ColorPickerDialog extends Dialog {
	...
	private static class ColorPickerView extends View {
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			if (action == MotionEvent.ACTION_CANCEL) {
				...
				startX = event.getX();
				startY = event.getY();
			} else if (pointerCount == 1 && action == MotionEvent.ACTION_MOVE
					&& !afterFirstDown) {
				afterFirstDown = true;
				startX = event.getX();
				startY = event.getY();
			...
			if (!afterFirstDown || pointerCount != 1
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sv.addView(new ColorPickerView(getContext(), l, mInitialColor,
				mDefaultColor), new LayoutParams(LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT));
		hsv.addView(sv, new LayoutParams(LayoutParams.WRAP_CONTENT,
				LayoutParams.MATCH_PARENT));
		setContentView(hsv, new LayoutParams(LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT));
